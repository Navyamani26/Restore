...
export default function CheckoutStepper() {
    ...
    const [addressComplete,setAdressComplete] = useState(false);
    const [paymnetComplete,setPaymnetComplete] = useState(false);
    ...
    const confirmPayment = async () => {
        ...
        try {
            ...
            const orderModel = await createOrderModell();
    ...
    const handleAdressChange = (event: StripeAddressElementChangeEvent) => {
        setAdressComplete(event.complete)
    }

    const handlePaymnetChange = (event: StripePaymentElementChangeEvent) => {
        setPaymnetComplete(event.complete)
    }

    if (isLoading) return <Typography variant="h6">  Loading chekcout ... </Typography>
  return (
    <Paper sx={{p:3, borderRadius:3}}>
        ...
        <Box sx={{mt:2}}>
            ...
            <Box sx={{display: activeStep === 1? 'block': 'none'}}>
                <PaymentElement onChange={handlePaymnetChange}
                options={{
                    wallets:{
                        applePay:'never',
                        googlePay:'never'
        ...
        <Box display='flex' paddingTop={2} justifyContent='space-between'>
            ...
            disabled={
                ...
                (activeStep === 1 && !paymnetComplete) ||
                submitting
            }
            loading={submitting}
            >
                {activeStep === steps.length - 1 ? `Pay ${currencyFormart(total)}` : 'Next'}
            </LoadingButton>
        </Box>
