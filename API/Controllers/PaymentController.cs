...
public class PaymentController(PaymentServices paymentService,
StoreContext context,IConfiguration config,ILogger<PaymentController> logger)
 : BasicApiController
{
    [Authorize]
    [HttpPost]
        ...
        foreach (var item in order.OrderItems)
        {
            var productItem = await context.Products
                .FindAsync(item.ItemOrdered.ProductId)
                    ?? throw new Exception("Problem udaitng order stock");
       }
